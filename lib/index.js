import { addEvent, getMods, getKeys, compareArray } from './utils.js'
import { _keyMap, _modifier, modifierMap, _mods, _handlers } from './var.js'

let _downKeys = [] // Records the binding key pressed
...
// set get current scope (default 'all')
...
function clearModifier (event) {
  ...
  // Special treatment cmmand key, the problem that the combination of shortcut keys keyup in cmmand is only executed once
  if (event.key && event.key.toLowerCase() === 'meta') {
    _downKeys.splice(0, _downKeys.length)
  }

  // modifier keys shiftKey altKey ctrlKey (command||metaKey) clear
...
function hotkeys (key, option, method) {
  _downKeys = []
  const keys = getKeys(key) // List of shortcut keys to handle
  let mods = []
